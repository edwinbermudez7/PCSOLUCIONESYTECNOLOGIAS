...
class ProductFormEdit extends Component {
    ...
    saveProductos = (e) => {
        ...
        if (this.validator.allValid()) {
            ...
            axios.put(this.url + 'product/'+this.productId, this.state.productos)
                .then(res => {
                    if (res.data.product) {
                        this.setState({
                            productos: res.data.product,
            ...
            <div id="productForm">
                ...
                <div className="center">
                    <div id="content">
                        ...
                        {this.state.productos.title &&
                            <form className="mid-form" onSubmit={this.saveProductos} >
                                <div className="form-group">
                                    <label htmlFor="title">Nombre del Producto</label>
                                    <input type="text" name="title" defaultValue={this.state.productos.title} ref={this.titleRef} onChange={this.changeState} />
                                    {this.validator.message('title', this.state.productos.title, 'required|alpha_num_space')}
                                </div>

                                <div className="form-group">
                                    <label htmlFor="description">Descripcion</label>
                                    <textarea name="description" defaultValue={this.state.productos.description} ref={this.descriptionRef} onChange={this.changeState} ></textarea>
                                    {this.validator.message('description', this.state.productos.description, 'required')}
                                </div>

                                <div className="form-group">
                                    <label htmlFor="price">Precio</label>
                                    <input type="number" name="price" defaultValue={this.state.productos.price} ref={this.priceRef} onChange={this.changeState} />
                                    {this.validator.message('price', this.state.productos.price, 'required|numeric|min:0,num')}
                                </div>

                                <div className="form-group">
                                    <label htmlFor="stop">Cantidad</label>
                                    <input type="number" name="stop" defaultValue={this.state.productos.stop} ref={this.stopRef} onChange={this.changeState} />
